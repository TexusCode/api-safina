<div class="space-y-5">
    ...
    <div class="grid grid-cols-2 gap-3">
        <flux:modal.trigger name="add-category">
            <button type="button"
                class="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-medium
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="rounded-xl border border-neutral-700 bg-neutral-800/60 p-4">
            ...
            <p class="text-2xl font-bold text-white">{{ number_format($monthTotal, 0, '.', ' ') }}<span
                    class="text-sm font-normal text-neutral-400 ml-1">c</span></p>
        </div>
        <div class="rounded-xl border border-neutral-700 bg-neutral-800/60 p-4">
            <p class="text-xs text-neutral-400 mb-1">По фильтру</p>
            <p class="text-2xl font-bold text-white">{{ number_format($filteredTotal, 0, '.', ' ') }}<span
                    class="text-sm font-normal text-neutral-400 ml-1">c</span></p>
        </div>
    </div>
    ...
    <div class="rounded-xl border border-neutral-700 bg-neutral-800/60 p-4 space-y-3">
        <div class="flex items-center justify-between">
            ...
            <button type="button" wire:click="resetFilters"
                class="text-xs text-blue-400 hover:text-blue-300">Сбросить</button>
        </div>
        <div class="grid grid-cols-2 gap-3">
    ...
    <div class="space-y-2">
        @forelse ($rashod as $item)
            <div
                class="rounded-xl border border-neutral-700/70 bg-neutral-800/50 p-4 hover:border-neutral-600 transition-colors duration-150">
                <div class="flex items-start justify-between gap-3">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            <span
                                class="text-lg font-semibold text-white">{{ number_format($item->price, 0, '.', ' ') }}c</span>
                            <span
        ...
        @empty
            <div class="rounded-xl border border-neutral-700/50 bg-neutral-800/30 p-8 text-center">
                <svg class="size-12 mx-auto text-neutral-600 mb-3" xmlns="http://www.w3.org/2000/svg"
    ...
    <flux:modal name="add-category">
        <div class="space-y-4">
            ...
            <div class="space-y-1 max-h-64 overflow-y-auto">
                @foreach ($categories as $cat)
                    <div
                        class="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-neutral-800/60 group transition-colors">
                        <span class="text-sm text-neutral-300">{{ $cat->name }}</span>
                        @if (Auth::user()->role == 'admin')
                            <button type="button" wire:confirm="Удалить категорию '{{ $cat->name }}'?"
