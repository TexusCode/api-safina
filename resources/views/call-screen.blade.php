...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .top-phone { font-size: 18px; font-weight: 700; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .close-btn {
            font-size: 13px; color: #6b7280; padding: 6px 10px;
            background: #1e1e28; border-radius: 8px; cursor: pointer;
            flex-shrink: 0;
        }

        /* ── SCROLLABLE BODY ──────────────────── */
        .scroll-body { flex: 1; overflow-y: auto; padding: 10px 12px; }

        /* ── CARDS ────────────────────────────── */
        .card {
            background: #141418; border: 1px solid #232330;
        ...
        .no-client-icon { font-size: 28px; margin-bottom: 4px; }

        /* ── END CALL BUTTON (phase 1 footer) ─── */
        .end-btn {
            width: 100%; background: #dc2626; border: none; border-radius: 12px;
            padding: 12px; color: #fff; font-size: 14px; font-weight: 600;
            cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px;
        }
        .end-btn:active { background: #b91c1c; }
        ...
        /* ── SUCCESS ──────────────────────────── */
        #phase-success { display: none; flex-direction: column; align-items: center; justify-content: center; height: 100%; text-align: center; padding: 20px; }
...
<div class="top-strip">
    ...
    <div class="top-info">
        <div class="top-label">
            @if($type === 'outgoing') Исходящий @else Входящий @endif
        </div>
        <div class="top-phone">{{ $phone ?: '—' }}</div>
    </div>
    <div class="close-btn" onclick="tryClose()">✕</div>
</div>

{{-- PHASE 1: INFO (during call) --}}
<div id="phase-info" style="display:flex; flex-direction:column; flex:1; overflow:hidden;">
...
{{-- PHASE 2: OUTCOME --}}
<div id="phase-outcome" style="flex:1; overflow:hidden; display:none; flex-direction:column;">
    <div class="scroll-body">
        @if($customer)
...
<div id="phase-success">
    ...
    <div class="success-sub" style="margin-top:4px">Закрывается...</div>
</div>
...
<script>
    ...
    // Called by Android JS injection when call ends
    ...
    function saveOutcome() {
        ...
        .then(function(data) {
            if (data.success) {
                ...
                setTimeout(function() { tryClose(); }, 2000);
            } else {
                btn.disabled = false;
